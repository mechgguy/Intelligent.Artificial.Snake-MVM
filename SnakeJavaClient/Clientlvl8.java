...
class Client {
    ...
    private void checkAndActivateSpeedBoostTrap(Position myHead, Direction chosenDir, GameField field, Snake mySnake, Set<Position> blocked) {
        ...
        // 1. SICHERHEIT: Wir dürfen uns nicht selbst killen!
        // Sind die nächsten 2 Schritte in der gewählten Richtung frei?
        ...
        if (blocked.contains(step1) || blocked.contains(step2)) {
            return; // Zu eng! Kein Boost!
        }

        // 2. FALLE: Ist ein Gegner-Kopf auf Distanz 2 oder 3?
        for (Map.Entry<String, Snake> e : field.snakesPerTeamName().entrySet()) {
            if (e.getKey().equals(teamName)) continue;
            Snake enemy = e.getValue();
            ...
            // Gegner ist gefährlich nah -> Boost zünden, um ihm den Körper in den Weg zu legen!
            if (dist > 0 && dist <= 3) {
    ...
    private Direction decideNormalDirection(
        ...
        for (Direction dir : Direction.values()) {
            ...
            // NEU: Bad Apple Opfer für das Schwert!
            boolean isSacrificeForSword = false;
            if (targetAttackItem != null && isSwordItem(targetAttackItem)) {
                // Wenn der Bad Apple direkt neben dem Schwert liegt
                if (distance(next, targetAttackItem.position(), field.size()) == 1) {
                }
            }
            ...
            if (badApples.contains(next)) {
                if (isSacrificeForSword) {
                    ...
                    // KEIN 'continue' hier! Wir erlauben der Schlange, das Feld normal zu bewerten.
                    // Die hohen Pluspunkte für das nahe Schwert ziehen sie jetzt automatisch durch den Bad Apple!
                } else {
