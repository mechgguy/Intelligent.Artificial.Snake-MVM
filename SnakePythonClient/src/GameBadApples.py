        ...
        try:
            ...
            if not chosen_move:
                if current_length < LENGTH_THRESHOLD:
                    ...
                    # Treat bad apples as open routing channels if length > 1
                    ...
                    if safe_moves:
                        ...
                        # If a good apple is unreachable, check if we can shortcut through a bad apple
                        if not chosen_move and bad_apples:
                            chosen_move = find_bfs_move(my_head, dangerous_cells, grid_size, bad_apples)
                else:
                    print(f"[SURVIVAL MODE] Preserving space loops at size {current_length}.")
                    # Proactively let survival mode drop through to the lookahead evaluator to calculate a move
            ...
            if chosen_move:
                ...
                # Absolute last resort fallback: keep moving straight if completely trapped
                print("[CRITICAL PANIC] No safe options. Maintaining lane trajectory.")
