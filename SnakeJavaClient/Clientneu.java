...
class Client {
    ...
    private Direction decideDirection(GameField field) {
        ...
        /*
         * WICHTIGER FIX: Verhindere, dass eigene Körperteile fälschlicherweise 
         * als Gegner gewertet werden, falls IDs oder Namen im Server variieren.
         */
        enemyBodies.removeAll(ownBody);

        /*
         * Wenn Sword/Star im Inventory ist UND direkt vorne eine echte gegnerische Snake steht.
         */
         ...
         */
        if (!hasAttackPower && !attackPreparedForNextTick && !isSpawning) {
    ...
    private Direction prepareAttackItemAndGoStraightIfEnemyInFront(
        ...
        if (sword != null && activateItemByName(sword)) {
            if (activateItemByName(sword)) {
                ...
                System.out.println("SWORD VORBEREITET UND GERADEAUS IN SNAKE: " + front);   // CHANGE HERE
                return trueDir;
            }
        }
        ...
        if (star != null && activateItemByName(sword)) {
            if (activateItemByName(star)) {
                attackPreparedForNextTick = true;
                System.out.println("STAR VORBEREITET UND GERADEAUS IN SNAKE: " + front);    // CHNAGE HERE   
                return trueDir;
            }
        }
    ...
    private boolean isSwordName(String name) {
        ...
        return name.toLowerCase().contains("sword")
                || name.toLowerCase().contains("schwert");
    }
    ...
    private Item findNearestGoodApple(Position head, List<Item> items, Size size) {
        ...
        for (Item item : items) {
            ...
            if (t.contains("apple") || t.contains("food")) {
